...
LSTM18 apply dropout only ont last value
LSTM19 add pass to LSTM"""
import os
import sys
...
conv_chan = 10                                         #number of kernel for convolution
conv_strides = int(np.ceil(num_step/num_LSTM))                  #decay between two convolution
conv_size = 15                                         #filter size for the convolution
...
with G.as_default():
    ...
    dataReduced = tf.layers.conv1d(inputs = dataShaped,filters = conv_chan,kernel_size = conv_size,strides=conv_strides,padding = "same",activation=None)#batch_size num_Lstm num_channel
    firstDataShaped = tf.slice(data,[0,0],[tf.shape(data)[0],tf.shape(dataReduced)[1]])
    firstDataShaped = tf.reshape(firstDataShaped,[tf.shape(data)[0],tf.shape(dataReduced)[1],1])
    dataReduced = tf.concat([dataReduced,firstDataShaped],axis = 2)
    def lstm_cell():
...
with tf.Session(graph=G) as sess:
    ...
    input_binary = True
    output_node_names = "FCPred/prediction"
...
num_step = 100                                        # time step before reduction
num_LSTM = 10                                         # num LSTM (time step after reduction)
conv_chan = 1                                         #number of kernel for convolution
strides = int(np.ceil(num_step/num_LSTM))                  #decay between two convolution
conv_size = 10                                         #filter size for the convolution
num_hidden = 200                                      #num of hidden units
num_class = 1                                          #size of the output
num_feature = 1                                        # size of the input
batch_size = 600                                   # number of sequence taken before to compute the gradient
...
with G.as_default():
    ...
    with tf.name_scope("convolutionalLengthReduction"):
        dataReduced = tf.layers.conv1d(inputs = dataShaped,filters = conv_chan,kernel_size = [conv_size],strides=strides,padding = "same",activation=None)
