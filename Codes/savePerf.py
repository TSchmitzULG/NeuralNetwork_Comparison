...
def logPerf(infoLog):
    ...
    with open(nameLogFile,'w') as mon_fichier:
        ...
        mon_fichier.write("Amplifier Name %s" %infoLog["amplifierName"]+'\n')
        ...
        for item in infoLog["conv_chan"]:
            mon_fichier.write("number of convolution kernels {}".format(item))
        
        mon_fichier.write("Convolution kernels size %f" %infoLog["conv_size"]+'\n')
        mon_fichier.write("strides size %f" %infoLog["strides"]+'\n')
