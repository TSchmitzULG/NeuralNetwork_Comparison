#!/usr/bin/python
# -*- coding: utf-8 -*-
"""LSTM20 simple conv layer
LSTM21 add pool and multi conv layers"""
import os
import sys
...
conv_chan = 20                                         #number of kernel for convolution
conv_strides = 3#int(np.ceil(num_step/num_LSTM))                  #decay between two convolution
conv_size = 7                                         #filter size for the convolution
size_poll = 3
...
trainDuration = 60*60*24*2                             # or during a determined duration(second)
...
with G.as_default():
    ...
    with tf.name_scope("ConvLayers"):
        ...
        dataReduced = tf.layers.conv2d(inputs = dataReduced,filters = conv_chan,
                                       kernel_size = (conv_size,1),strides=(conv_strides,1),
                                       padding = "same",activation=None,name="C3")#batch_size num_Lstm num_channel
        #dataReduced = tf.nn.max_pool(dataReduced,ksize=[1,size_poll,1,1],strides=[1,2,1,1],
