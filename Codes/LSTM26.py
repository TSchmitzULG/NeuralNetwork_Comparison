...
with G.as_default():
    ...
    dataShaped1,dataShaped2 = tf.split(dataShaped,num_or_size_splits=2,axis=1)
    def lstm_cell():
        return tf.contrib.rnn.LSTMCell(num_hidden,state_is_tuple=True,activation = tf.nn.tanh)
    

    with tf.variable_scope("LSTM1_1"):
        val, state = tf.nn.dynamic_rnn(lstm_cell(), dataShaped1, dtype=tf.float32) # val dim is [batchSize,num_step, numhidden]
        ...
        with tf.variable_scope("FCLayer1_1"):
            ...
            out1_1 = tf.nn.elu((tf.add(tf.matmul(last, weight) , bias)),name = "out1") #[batchSize,nclass]
    
    with tf.variable_scope("LSTM1_2"):
        val, state = tf.nn.dynamic_rnn(lstm_cell(), dataShaped2, dtype=tf.float32) # val dim is [batchSize,num_step, numhidden]
        last_index = tf.shape(val)[1] - 1 # Then let's reshape the output to [sequence_length,batch_size,numhidden]
        val = tf.transpose(val,[1,0,2])    # Last state of all batches
        last = tf.gather(val,last_index)# tensor [batchsize,numhidden]
        with tf.variable_scope("FCLayer1_2"):
            last = dropout(last,keep_prob, is_training = is_training)
            regularizerFC1 = tf.contrib.layers.l1_l2_regularizer(scale_l1=reg_scale_l1,scale_l2=reg_scale_l2,scope="regFC1")
            weight = tf.get_variable("weight", shape=[num_hidden, int(target.get_shape()[1])],regularizer=regularizerFC1, initializer=tf.contrib.layers.xavier_initializer())
            bias = tf.Variable(tf.constant(0., shape=[target.get_shape()[1]]))
            out1_2 = tf.nn.elu((tf.add(tf.matmul(last, weight) , bias)),name = "out2") #[batchSize,nclass]
    outLayer1 = tf.concat([out1_1,out1_2],axis=1)
    outLayer1 = tf.reshape(outLayer1,[tf.shape(data)[0],2,1])
    
    with tf.variable_scope("LSTM2_1"):
        val, state = tf.nn.dynamic_rnn(lstm_cell(), outLayer1, dtype=tf.float32) # val dim is [batchSize,num_step, numhidden]
        last_index = tf.shape(val)[1] - 1 # Then let's reshape the output to [sequence_length,batch_size,numhidden]
        val = tf.transpose(val,[1,0,2])    # Last state of all batches
        last = tf.gather(val,last_index)# tensor [batchsize,numhidden]

    with tf.variable_scope("FCLayer2_1"):
        last = dropout(last,keep_prob, is_training = is_training)
        regularizerFC1 = tf.contrib.layers.l1_l2_regularizer(scale_l1=reg_scale_l1,scale_l2=reg_scale_l2,scope="regFC1")
        weight = tf.get_variable("weight", shape=[num_hidden, int(target.get_shape()[1])],regularizer=regularizerFC1, initializer=tf.contrib.layers.xavier_initializer())
        bias = tf.Variable(tf.constant(0., shape=[target.get_shape()[1]]))
