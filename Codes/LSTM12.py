...
with G.as_default():
    ...
    dataDrop = dropout(data,keep_prob, is_training = is_training)	
    dataReduced = fully_connected(dataDrop,num_LSTM,activation_fn=tf.nn.elu,scope="dataReduced")
    dataShaped = tf.reshape(dataReduced,[tf.shape(data)[0],num_LSTM,num_feature])
...
with tf.Session(graph=G) as sess:
    ...
    for epoch in range(num_epoch):
        if (time.clock()-tStart < trainDuration):
            ...
            print("Epoch {} RMSE {} on test set".format(epoch,np.sqrt(pMSE/no_of_batchesTest)))
