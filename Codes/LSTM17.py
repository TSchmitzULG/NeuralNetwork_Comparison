...
LSTM15 add dropout 
LSTM17 add kernels regularizer
"""
import os
import sys
...
with G.as_default():
    ...
    dataShaped = dropout(dataShaped,keep_prob, is_training = is_training)
    with tf.name_scope("convolutionalLengthReduction"):
        regularizer = tf.contrib.layers.l2_regularizer(scale=0.1)
        dataReduced = tf.layers.conv1d(inputs = dataShaped,filters = conv_chan,kernel_size = [conv_size],strides=strides,padding = "same",kernel_regularizer=regularizer,activation=None)
    ...
    loss = tf.add_n([MSE]+reg_losses,name="regularizedLoss")
