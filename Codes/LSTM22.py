...
num_LSTM=10
# Hyper parameters
def generate_random_hyperparams(num_step_min,num_step_max,conv_chan_min, conv_chan_max,conv_size_min,conv_size_max):
    '''generate random learning rate and keep probability'''
    random_num_step = np.random.randint(num_step_min, num_step_max+1)
    random_conv_chan = np.random.randint(conv_chan_min, conv_chan_max+1)
    random_conv_size = np.random.randint(conv_size_min, conv_size_max+1)
    conv_strides = int(np.ceil(np.sqrt(random_num_step/num_LSTM)))
    return random_num_step,random_conv_chan, random_conv_size,conv_strides

trainTestRatio = 0.9 #if you cannot load all the data set in Ram specify wich part you want to load (0 means all the dataset)
maxSize = 0
num_step_min = 130                                        # time step before reduction
num_step_max = 160
conv_chan_min = 25                                         #number of kernel for convolution
conv_chan_max = 50
#conv_strides = 3#int(np.ceil(num_step/num_LSTM))                  #decay between two convolution
conv_size_min = 10                                         #filter size for the convolution
conv_size_max = 15
...
trainDuration = 60*60*3                            # or during a determined duration(second)
...
num_hyperparam_explore = 6
params = np.zeros((num_hyperparam_explore,4),np.int)
for i in range(num_hyperparam_explore): 
    params[i] = generate_random_hyperparams(num_step_min,num_step_max,conv_chan_min, conv_chan_max,conv_size_min,conv_size_max)
...
def run_model(num_step,conv_chan,conv_size,conv_strides):
    ...
    with G.as_default():
        ...
        with tf.name_scope("ConvLayers"):
            dataReduced = tf.layers.conv2d(inputs = dataShaped,filters = conv_chan,
                                           kernel_size = (conv_size,1),strides=(conv_strides,1),
                                           padding = "same",activation=None,name="C1")#batch_size num_Lstm num_channel
            #dataReduced = tf.nn.max_pool(dataReduced,ksize=[1,size_poll,1,1],strides=[1,2,1,1],
            #                             padding="VALID",name="S2")
            dataReduced = tf.layers.conv2d(inputs = dataReduced,filters = conv_chan,
                                           kernel_size = (conv_size,1),strides=(conv_strides,1),
                                           padding = "same",activation=None,name="C2")#batch_size num_Lstm num_channel
            #dataReduced = tf.nn.max_pool(dataReduced,ksize=[1,size_poll,1,1],strides=[1,2,1,1],
        ...
        with tf.name_scope("extractLastValueLSTM"):
            val, state = tf.nn.dynamic_rnn(multiLayerCell, dataReduced, dtype=tf.float32) # val dim is [batchSize,num_step, numhidden]
    ...
    with tf.Session(graph=G) as sess:
            ...
            if (time.clock()-tStart < trainDuration):
                ...
                for j in range(no_of_batches):
                    ...
                    if j % np.floor(trainTestRatio*10) ==0 : # This is to have a train summary and a test summary of the same size
                        _,summary_str = sess.run([minimize,summary_op],{data: inp, target: out,is_training: True})
                    ...
                    #print value
                timeOneEpoch = time.clock()-tEpoch
                #print ("Epoch -{} calculated in {} s ".format(epoch,timeOneEpoch))
        ...
        performance = np.sqrt(pMSE/no_of_batchesTest)
        #######################
    ...
    return performance,timeOneEpoch

performance_records = {}
for i in range(num_hyperparam_explore): # random search hyper-parameter space 10 times
    print("Parameters : numstep,convChan,convSize,convStrides: {}".format(params[i]))
    
    performance,timeOneEpoch = run_model(params[i][0],params[i][1],params[i][2],params[i][3])
    print(("Performance MSE = {} at step {} in {} s/epoch with parameters : numstep,convChan,convSize,convStrides {}".format(performance,i,timeOneEpoch,params[i])))
    performance_records[tuple(params[i])]= [performance,timeOneEpoch]
for i in performance_records:
    #print("MSE {} with batch_size = {}, num_step = {}, num_hidden = {}, n_layer = {}".format(performance_records[i],i[0],i[1],i[2],i[3])) 
    print("{},{},{},{},{},{},".format(performance_records[i][0],performance_records[i][1],i[0],i[1],i[2],i[3]))
