...
conv_strides = 1                     #decay between two convolution
conv_size = 3                                         #filter size for the convolution
size_poll = 4
...
with G.as_default():
    ...
    dataShaped = tf.reshape(data,[tf.shape(data)[0],num_step,num_feature]) # batchSize, num_step num_feature
    dataConv = tf.layers.conv1d(inputs = dataShaped,filters = conv_chan,kernel_size = conv_size,strides=conv_strides,padding = "same",activation=None)#batch_size num_Lstm num_channel
    dataConv = tf.reshape(dataConv,[tf.shape(data)[0],tf.shape(dataConv)[1],conv_chan,1])
    dataPoll = tf.nn.max_pool(dataConv,ksize=[1,size_poll,1,1],strides=[1,2,1,1],padding="VALID")
    dataPoll = tf.reshape(dataPoll,[tf.shape(data)[0],tf.shape(dataPoll)[1],conv_chan])
    def lstm_cell():
    ...
    with tf.name_scope("extractLastValueLSTM"):
        val, state = tf.nn.dynamic_rnn(multiLayerCell, dataPoll, dtype=tf.float32) # val dim is [batchSize,num_step, numhidden]
